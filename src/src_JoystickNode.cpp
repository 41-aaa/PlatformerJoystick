...
void JoystickNode::handleInput(GJBaseGameLayer *layer, CCPoint input, CCPoint old) {
    ...
    // Update the visual text label (guarded if label present)
    if (m_valueLabel) {
        std::string s = fmt::format("X: {:.3f} Y: {:.3f}", input.x, input.y);
        m_valueLabel->setString(s.c_str());
    }
}

void JoystickNode::fakePosition() {
    // Used for keyboard/controller input to sync the visual knob
    CCPoint pos = (getContentSize() / 2) + (m_currentInput * (getContentSize().width / 2));
    m_center->setPosition(pos);
    if (m_valueLabel) {
        std::string s = fmt::format("X: {:.3f} Y: {:.3f}", m_currentInput.x, m_currentInput.y);
        m_valueLabel->setString(s.c_str());
    }
}
...
void JoystickNode::ccTouchMoved(CCTouch *touch, CCEvent *event) {
    ...
    float maxRadius = (getContentSize().width / 2) - (m_center->getContentSize().width * m_center->getScale() / 2);
...
void JoystickNode::draw() {
    ...
    #if defined(DEBUG_BUILD) && !defined(GITHUB_ACTIONS)
    {
        ...
        auto radius = getScaledContentSize().width / 2;
...
class $modify(JSLEL, LevelEditorLayer) {
    ...
    void popup(bool bounce = true) {
        ...
        auto po = createQuickPopup("Warning", "Your jump keybind seems to be set to the same as your up keybind. If you don't change it, you will not be able to input up.", "Ok", "Keybind Config", [this](bool btn2) {
            if (btn2) {
                // :/
...
class $modify(JSPL, PlayLayer) {
    ...
    void popup(bool bounce = true) {
        ...
        queueInMainThread([bounce, this] {
            auto po = createQuickPopup("Warning", "Your jump keybind seems to be set to the same as your up keybind. If you don't change it, you will not be able to input up.", "Ok", "Keybind Config",[this, bounce](bool btn2) {
                if (btn2) {
                    // :/
                    (this->*menu_selector(MoreOptionsLayer::onKeybindings))(nullptr);
                } else {
