...
// Now accepts a floating value so decimals are preserved when showing/updating
void updateVal(GJBaseGameLayer *layer, int id, float val) {
    if (enableJoystick) {
        // pass floating values through; underlying methods should handle floats if supported
...
void JoystickNode::handleInput(GJBaseGameLayer *layer, CCPoint input, CCPoint old) {
    // interpret as continuous values now. Use threshold to determine directional button presses/releases.
    constexpr float threshold = 0.5f;
    auto signFor = [&](float v)->int {
        if (v > threshold) return 1;
        if (v < -threshold) return -1;
        return 0;
    };

    int oldSignX = signFor(old.x);
    int newSignX = signFor(input.x);
    if (oldSignX != newSignX) {
        if (oldSignX == 1) {
    ...
    if (layer->m_player1 && !layer->m_player1->m_controlsDisabled) {
        // only update if value changed by a meaningful amount (avoid spamming updates)
        if (std::abs(input.x - old.x) > 0.0001f) updateVal(layer, 3741, input.x);
        if (std::abs(input.y - old.y) > 0.0001f) updateVal(layer, 3742, input.y);
    }
}

void JoystickNode::fakePosition() {
    // compute the visual position from the current (possibly fractional) input
    float maxDist = getScaledContentWidth() / 2 - m_center->getScaledContentWidth() / 2;
    CCPoint pos = getContentSize() / 2 + m_currentInput * getContentWidth() / 2;

    auto fromCenter = pos - getScaledContentSize() / 2;

    if (fromCenter.getLength() > maxDist) {
        fromCenter = fromCenter.normalize() * maxDist;
        pos = fromCenter + getScaledContentSize() / 2;
    }

    // ensure the center sprite stays within allowed radius
...
void JoystickNode::ccTouchMoved(CCTouch *touch, CCEvent *event) {
    ...
    // produce continuous values between -1 and 1
    float rawX = fromCenter.x / maxDist;
    float rawY = fromCenter.y / maxDist;
    // clamp to [-1, 1]
    rawX = std::max(-1.0f, std::min(1.0f, rawX));
    rawY = std::max(-1.0f, std::min(1.0f, rawY));
    // deadzone so tiny noise doesn't register
    constexpr float deadzone = 0.12f;
    CCPoint inp = { std::abs(rawX) > deadzone ? rawX : 0.0f, std::abs(rawY) > deadzone ? rawY : 0.0f };
        ...
        queueInMainThread([this, gjbgl](){
            ...
            m_gameLayer->addEventListener<keybinds::InvokeBindFilter>([this](keybinds::InvokeBindEvent* event) {
                ...
                if (auto node = m_fields->m_joystickNode) {
                    ...
                    // clamp to [-1,1] so keyboard/joystick combination doesn't overflow
                    node->m_currentInput.y = std::max(-1.0f, std::min(1.0f, node->m_currentInput.y));
            ...
            m_gameLayer->addEventListener<keybinds::InvokeBindFilter>([this](keybinds::InvokeBindEvent* event) {
                ...
                if (auto node = this->m_fields->m_joystickNode) {
                    ...
                    node->m_currentInput.x = std::max(-1.0f, std::min(1.0f, node->m_currentInput.x));
                    node->handleInput(this->m_gameLayer, node->m_currentInput, old);
                    node->fakePosition();
                }
                return ListenerResult::Stop;
...
class $modify(JSLEL, LevelEditorLayer) {
    ...
    void popup(bool bounce = true) {
        ...
        auto po = createQuickPopup("Warning", "Your jump keybind seems to be set to the same as your up keybind. If you don't change it, you will not be able to input up.", "Ok", "Keybind Config", [th[...]
            if (btn2) {
                // :/
...
class $modify(JSPL, PlayLayer) {
    ...
    void popup(bool bounce = true) {
        ...
        queueInMainThread([bounce, this] {
            auto po = createQuickPopup("Warning", "Your jump keybind seems to be set to the same as your up keybind. If you don't change it, you will not be able to input up.", "Ok", "Keybind Config",[...]
                if (btn2) {
                    // :/
                    (this->*menu_selector(MoreOptionsLayer::onKeybindings))(nullptr);
                } else {
...
class $modify(JSPO, PlayerObject) {
    void disablePlayerControls() {
        ...
        updateVal(m_gameLayer, 3741, 0.0f);
        updateVal(m_gameLayer, 3742, 0.0f);
    }
    ...
    void enablePlayerControls() {
        ...
        if (!js) return;
        // ensure values are clamped and pass them as floats
        float vx = std::max(-1.0f, std::min(1.0f, js->m_currentInput.x));
        float vy = std::max(-1.0f, std::min(1.0f, js->m_currentInput.y));
        updateVal(m_gameLayer, 3741, vx);
        updateVal(m_gameLayer, 3742, vy);
    }
};
